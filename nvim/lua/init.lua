...
vim.keymap.set(
  ...
  function ()
    TelescopeFindFiles(
      {'rg','--files','--no-config'},
      {no_ignore=true, no_ignore_parent=true},
      {'~', vim.fn.getcwd()}
  )
  end,
  { desc='Telescope: All files - No respect for ignores' }
)

function TelescopeFindFiles(find_command, options, search_dirs)
  local expanded_search_dirs = vim.fn.map(search_dirs, 'expand(v:val)')
  require('telescope.builtin').find_files(vim.tbl_extend('keep', options, {
    find_command = vim.fn.executable(find_command[1]) == 1 and find_command or nil,
    hidden = true,
    follow = true,
    search_dirs = require('my_lua/util').remove_duplicates(expanded_search_dirs)
  }))
end
